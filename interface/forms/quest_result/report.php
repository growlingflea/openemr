...
//	$content = do_status($order_data->status, $order_data->priority);
	...
	$reviewed = ($result_data->reviewed_id) ? UserIdLook($result_data->reviewed_id) . " (" . date('Y-m-d',strtotime($result_data->reviewed_datetime)) . ") " : "";
	$content .= do_columns(UserIdLook($result_data->request_provider),'Ordering Provider', $reviewed,'Reviewed By');
	$notified = ($result_data->notified_id) ? UserIdLook($result_data->notified_id) . " (" . date('Y-m-d',strtotime($result_data->notified_datetime)) . ") " : "";
	if ($order_data->request_provider != $order_data->user) {
		$content .= do_columns(UserLook($order_data->user),'Entering Clinician', $notified,'Notification By');
	}
	else {
		$content .= do_columns('BLANK','', $notified,'Notification By');
	}
	$content .= do_columns('BLANK','', $result_data->notified_person, 'Person Notified');
	$content .= do_columns('BLANK','', ListLook($result_data->result_handling, 'Quest_Special_Handling'), 'Special Handling');
...
//	$content .= do_line($specimen,"Collected Date"); 
	...
	foreach ($item_list as $item_data) {
		...
		if ($item_data->observation_value || $item_data->observation_units) {
			if (!$abnormal) $newRow .= "<td class='wmtOutput mono' style='width:400px;padding-left:10px'>".$item_data->observation_value." ".$item_data->observation_units."\n";
			if ($abnormal) $newRow .= "<td class='wmtOutput mono' style='".$hilite."width:400px;white-space:pre-wrap'>".$abnormal." ".$item_data->observation_value."</td>\n";

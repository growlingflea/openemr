...
//DEBUG
//$message->order_number = '000000';
//$message->pid = '111111';
//$message->provider_id[0] = '222222';
//END DEBUG
	...
	*/
	...
	if ($message->notes) {
		foreach ($message->notes AS $note) {
			...
			$result_data->lab_notes .= $note->comment;
		}
	}
	...
	foreach ($message->orders as $order) {
		...
		foreach ($order->results as $result) {
			...
			$item_data->test_code = $order->service_id[0];
			$item_data->test_text = $order->service_id[1];
			$item_data->component_code = $order->component_id[0];
			$item_data->component_text = $order->component_id[4];
				
			$item_data->observation_type = $result->value_type;
			$item_data->observation_label = ltrim($result->observation_id[4]);
			$item_data->observation_value = $result->observation_value;
			$item_data->observation_units = $result->observation_units;
			$item_data->observation_range = $result->observation_range;
			$item_data->observation_status = $result->observation_status;
			$item_data->observation_abnormal = $result->observation_abnormal;
			$item_data->observation_datetime = $result->observation_datetime;
			$item_data->observation_notes = $notes;
	...
	if (count($items)) { // got results
		...
		if ($result_id) { // have an existing record
			if ($result_data->result_notes){
				//$result_data->result_notes = str_replace('\\\\n','\n',$result_data->result_notes);
				$result_data->result_notes = preg_replace('/\\\\+/','\\',$result_data->result_notes); 
				$result_data->result_notes = $result_data->result_notes."\n";
			}
	...
	$query .= "result_output = ? ";
	sqlStatementNoLog($query, array($output));
}

...
//		$newRow .= "</tr><tr><td colspan='5'></td>\n";
	...
	if ($item->observation_notes) { 
		$newRow .= "<td class='mono' colspan='2' style='white-space:pre-wrap;vertical-align:top;".$hilite."'>".str_replace('\n', "\n", $item->observation_notes)."</td></tr>\n";
	}
}
...
<?php 
	$rlist= sqlStatement("SELECT * FROM users WHERE authorized=1 AND active=1 AND npi != '' AND username != '' AND facility_id > 0 ORDER BY lname");
									...
									<tr>
										...
										<td>
											<select class="wmtInput" name='notified_id' id='notified_id' style="min-width:200px" onchange="$('#notified_date').val('<?php echo date('Y-m-d') ?>')">
												<option value='_blank'>-- select --</option>
<?php 
	$rlist= sqlStatement("SELECT * FROM users WHERE active=1 AND username != '' AND facility_id > 0 ORDER BY lname");
	while ($rrow= sqlFetchArray($rlist)) {
    	echo "<option value='" . $rrow['id'] . "'";
								...
								<table style="width:100%">
									<tr>
										<td class="wmtLabel" colspan="3">
											Clinical Notes: 
											<textarea name="result_notes" id="result_notes" class="wmtFullInput" rows="4"><?php echo $result_data->result_notes ?></textarea>
										</td>
									</tr>
								</table>
							</td>

 ...
 *  @uses labcorp_result/update.php
 * 
 *************************************************************************** */
...
$form_title = 'LabCorp Result Link';
$form_name = 'labcorp_link';
$form_id = $_REQUEST['id'];
...
function labPnote($pid, $newtext, $assigned_to = '', $datetime = '') {
	...
	$body = date('Y-m-d H:i') . ' (LabCorp Labs to '. $assigned_to;
	$body .= ') ' . $newtext;
...
$result_data = new wmtForm($result_name,$form_id,TRUE);
...
if ($process) { // doing the work
	...
	$query .= "WHERE o.list_id = 'LabCorp_Site_Identifiers' AND o.title = '$result_data->request_facility' ";
	...
	if ($site = sqlQuery($query)) {
		...
		$query = "SELECT o.option_id, o.title, f.name, f.id FROM list_options o, facility f ";
		$query .= "WHERE o.list_id = 'LabCorp_Site_Identifiers' AND f.id = '$provider_facility' ";
		$query .= "AND o.option_id = f.id ";
		if ($site = sqlQuery($query)) {
			$site_id = $site['id'];
			$site_name = $site['name'];
	...
	if ($result_data->document_id) { // only continue if there is a document
	...
	addForm($encounter, "LABCORP RESULT ENCOUNTER",
		...
		"newpatient", $form_pid, 0, date('Y-m-d'), 'labcorp');
	...
	if ($provider_username && $provider_username != 'labcorp') {
		$link_ref = "$rootdir/forms/labcorp_result/update.php?id=$result_data->id&pid=".$form_pid."&enc=".$encounter;
		$note = "\n\nLabCorp results for order number '".$result_data->request_order."' have been manually linked to your patient '".$pat_data->fname." ".$pat_data->lname."' (pid: ".$form_pid."). ";
...
<?php if (!$process) { ?>
			...
			<br/><br/>
			A new encounter will be created for this patient and the LabCorp documents and result information<br/>
...
<?php } else { ?>
			...
			<br/><br/>
			Encounter #<?php echo $encounter ?> has been created for this patient and the LabCorp documents and result information<br/>

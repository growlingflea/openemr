...
if ($GLOBALS['wmt_lab_icd10']) {
	$query = "SELECT title, notes, CONCAT('ICD10:',formatted_dx_code) AS code, short_desc, long_desc FROM list_options l ";
...
foreach ($dlist as $data) {
	...
	echo "<tr><td style='white-space:nowrap'><nowrap><input class='wmtCheck' type='checkbox' id='check_".$id."' code='".$data['code']."' desc='".htmlspecialchars($text)."' > <b>".$code."</b></input> - </nowrap></td><td style='padding-top:0'>".$text."</td></tr>\n";
}
...
<?php 
$title = 'Search';
echo "<div class='wmtQuick' id='oc_Search' style='display:none;margin-left:0'><table width='100%'><tr><td style='text-align:center;padding-top:30px'><h3>Select profile at left or<br/>search using search box at top.</h3>\n";
foreach ($olist as $data) {
	if ($data['title'] != $title) {
		if ($title) echo "</table></div>\n"; // end previous section
		$title = $data['title']; // new section

...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="<?php echo $GLOBALS['webroot'] ?>/interface/forms/quest_order/style.css" media="screen" />
...
//$client->buildRequest(10,'03/15/2013','03/20/2013');
...
//END DEBUG
	...
	*/
	...
	$result_data = new wmtForm($result_name, $result_id); // will create if no id present
	...
	if (count($items)) { // got results
		...
		if ($result_id) { // have an existing record
			...
			$result_data->result_notes .= "RESULTS REVISED: ".date('Y-m-d H:i:s')." - Any previous review and notification data cleared !!";
